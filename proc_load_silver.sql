...
CREATE OR ALTER PROCEDURE silver.load_silver AS
	...
	FROM(
		...
		*,
        ...
        -- Loading siver.crm_sales_details
        SET @start_time = GETDATE();
		...
		CASE WHEN sla_sales IS NULL OR sla_sales <=0 OR sla_sales != sls_quantity * ABS(sls_price)
			 ...
			 ELSE sla_sales
		END AS sla_sales, ---Recalculate sales if original value is missing or incorrect
		sls_quantity,
		CASE WHEN sls_price IS NULL OR sls_price <=0
				 THEN sla_sales / NULLIF(sls_quantity,0)
			  ELSE sls_price
		END AS sls_price ---- Derive price if original value is inavlid
